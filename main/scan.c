	...
	switch (type)
	{
		case WIFI_PKT_MGMT:
			...
			if(frame->frame_control.subtype == 4)
			{
				...
				{
					if(tagLst.content[i].tag_number == SUPPORTED_RATES && pri.supported_rates.tag_length == 0)
					{
				...
				if(res == NULL && pri.supported_rates.tag_length == 8)
					free(pri.supported_rates.values);
				else
				{
					if(detectedProbeRequests.objCount % 100 == 0)
					{
